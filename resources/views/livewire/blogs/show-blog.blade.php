    ...
    <div class="mx-auto grid lg:grid-cols-2 gap-6">
        @if($this->prev)
            <div
                ...
                <a href="{{ route('blogs.show', $this->prev) }}">
                    <div class="flex">
                        <span class="text-primary-600">Previous blog </span>
                    </div>

                    <div class="text-gray-700">{{ $this->prev->title }}</div>
                </a>
            </div>
        @else
            <div></div>
        ...
        @if($this->next)
            <div
                class="cursor-pointer rounded-lg border border-gray-300 transition hover:border-gray-500 hover:bg-gray-50 p-2">
                <a href="{{ route('blogs.show', $this->next) }}">
                    <div class="flex justify-end">
                        <span class="text-primary-600">Next blog </span>
                    </div>

                    <div class="flex justify-end text-right text-gray-700">{{ $this->next->title }}</div>
                </a>
            </div>
        @endif
    </div>
